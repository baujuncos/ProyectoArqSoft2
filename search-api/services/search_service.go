...
type Repository interface {
	Index(ctx context.Context, curso coursesDTO.CourseDto) (string, error)
	Update(ctx context.Context, curso coursesDTO.CourseDto) error
	Search(ctx context.Context, query string, limit int, offset int) ([]coursesDTO.CourseDto, error)
}

// ExternalRepository define el método para obtener un curso de la API externa
type ExternalRepository interface {
	GetCourseByID(ctx context.Context, id string) (coursesDTO.CourseDto, error)
}
...
func (service Service) Search(ctx context.Context, query string, offset int, limit int) ([]coursesDTO.CourseDto, error) {
	...
	cursosDomainList := make([]coursesDTO.CourseDto, 0)
	for _, curso := range cursosDAOList {
		cursosDomainList = append(cursosDomainList, coursesDTO.CourseDto{
...
func (service Service) HandleCourseNew(cursoNew coursesDTO.CourseNew) {
	...
	case "CREATE", "UPDATE":
		...
		if cursoNew.Operation == "CREATE" {
			if _, err := service.repository.Index(context.Background(), curso); err != nil {
				...
				fmt.Println("Course indexed successfully:", cursoNew.Course_id)
			}
		} else {
			if err := service.repository.Update(context.Background(), curso); err != nil {
				fmt.Printf("Error updating course (%s): %v\n", cursoNew.Course_id, err)
			} else {
				fmt.Println("Course updated successfully:", cursoNew.Course_id)
			}
		}
...
func (repo *SolrRepository) Index(ctx context.Context, curso coursesDTO.CourseDto) (string, error) {
