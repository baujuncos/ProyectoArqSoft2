...
const CreateCourse = ({ onClose }) => {
    ...
    const profesor_id = String(Cookies.get('user_id')); // Garantizar que es string
    ...
    const handleSubmit = async (e) => {
        ...
        const data = {
            nombre,
            profesor_id, // Garantizado como string
            categoria,
            descripcion,
        ...
        try {
            const response = await fetch('http://localhost:8081/courses', {
                ...
                headers: {
                    ...
                    'Authorization': `Bearer ${tokenUser}`, // Si se necesita autenticación
                },
            ...
            if (!response.ok) {
                ...
                setErrorMessage(`Error al crear el curso: ${errorData.message || 'Solicitud incorrecta.'}`);
            } else {
            ...
            setErrorMessage('Error de red al crear el curso.');
        }
    };
