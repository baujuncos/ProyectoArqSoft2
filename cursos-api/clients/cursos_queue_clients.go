...
func (queue Rabbit) Publish(coursoNew coursesDTO.CourseNew) error {
	bytes, err := json.Marshal(coursoNew)
	if err != nil {
		return fmt.Errorf("error marshaling Rabbit coursoNew: %w", err)
	}
