...
type Repository interface {
	...
	GetUserByUsername(mail string) (dao.Users, error)
...
func (service Service) GetUserByEmail(email string) (domain.User, error) {
	// Check in cache first
	...
	// Check memcached
	user, err = service.memcachedRepository.GetUserByEmail(user)
	if err == nil {
		if _, err := service.cacheRepository.CreateUser(user); err != nil {
			return domain.User{}, fmt.Errorf("error caching user after memcached retrieval: %w", err)
		}
...
func (service Service) CreateUser(user domain.User) (int64, error) {
	// Hash the password
	passwordHash := Hash(user.Password)
